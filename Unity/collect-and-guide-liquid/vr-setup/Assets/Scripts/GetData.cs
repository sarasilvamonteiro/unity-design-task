...
public class GetData : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        ...
        if (gameObject.GetComponent<CreateVR>().start)
        {
            InitializeObjects();
            gameObject.GetComponent<CreateVR>().start = false;
            gameObject.GetComponent<CreateVR>().DisableButton();
            gameObject.GetComponent<CreateVR>().StartSimulation();
        }


        if (running && frames % 15 == 0)
        {
            // running is false when fluid stops. This means data is no longer recorded after this.
            Debug.Log("get data running");
    ...
    private void InitializeObjects()
    {
        ...
        holes = gameObject.GetComponent<CreateVR>().holes;
        Debug.Log(holes);
        flows = gameObject.GetComponent<CreateVR>().flows;
        Debug.Log(flows);
        useHands = gameObject.GetComponent<CreateVR>().useHands;
        emptyTrial = gameObject.GetComponent<CreateVR>().emptyTrial;

        // Fluid:
        totalParticles = gameObject.GetComponent<CreateVR>().totalParticles;
        totalEmittedParticles = 0;
        totalHoleParticles = 0;
        totalWastedParticles = 0;
        singleHoleAllParticles = Vector3.zero;        
        
        // (maybe remove this - not checked for Jul. 23)
        ...
        clayContainer = gameObject.GetComponent<CreateVR>().MainObject;

        if (!emptyTrial)
        {
            emitterContainer = gameObject.GetComponent<CreateVR>().emitterContainer;
            groundContainer = gameObject.GetComponent<CreateVR>().groundContainer;
            liquidContainer = gameObject.GetComponent<CreateVR>().liquidContainer;
    ...
    private void SaveInitialPositions()
    {
        ...
        Dictionary<int, int> SphereIDs = gameObject.GetComponent<CreateVR>().SphereIDs;
        ...
        generalRow[0] = "Agent";
        generalRow[1] = Vector3.zero;//personController.transform.position; // Player position
        generalRow[2] = GameObject.FindObjectOfType<EyeTracking>().fixationPointTransform.localPosition;//Vector3.zero;//new Vector3(personController.transform.GetChild(0).transform.eulerAngles.x, personController.transform.eulerAngles.y, 0); // Player rotation
    ...
    private DataTable CreateSphereTable()
    {
        ...
        Dictionary<int, int> SphereIDs = gameObject.GetComponent<CreateVR>().SphereIDs;
        Rigidbody[] sphere = clayContainer.GetComponentsInChildren<Rigidbody>(); // has length of number of spheres in material
    ...
    private void CountParticles()
    {
        ...
        {
            getEmitterParticles += emitter.CreatedParticlesTotal;
    ...
    private void GeneralData()
    {
        ...
        // Player:
        row[0] = Vector3.zero;//personController.transform.position;// Player position
        row[1] = GameObject.FindObjectOfType<EyeTracking>().fixationPointTransform.localPosition;//new Vector3(personController.transform.GetChild(0).transform.eulerAngles.x, personController.transform.eulerAngles.y, 0); // Player looking at
        // Object:
    ...
    private void OnApplicationQuit()
    {
        ...
        if (emptyTrial)
        {
            ...
            flows = 0;
            totalParticles = 0;
            // Save empty trial data
            SaveData(emptyTrial: true, path: path, idx: "empty");
        ...
        if (!emptyTrial)
        {
            ...
            // Save normal trial data
            SaveData(emptyTrial: false, path: path, idx: idx.ToString());
    ...
    private void SaveData(bool emptyTrial, string path, string idx)
    {
